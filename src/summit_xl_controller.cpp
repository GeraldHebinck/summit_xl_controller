...
namespace summit_xl_controller
{
    ...
    bool SummitXLController::init(hardware_interface::VelocityJointInterface* hw,
            ...
            ros::NodeHandle &controller_nh)
    {
        ...
        controller_nh.param<std::string>("joint/front_right_wheel_joint/name", front_right_traction_joint_, "summit_xl_front_right_wheel_joint");
        controller_nh.param<std::string>("joint/front_left_wheel_joint/name", front_left_traction_joint_, "summit_xl_front_left_wheel_joint");
        controller_nh.param<std::string>("joint/back_left_wheel_joint/name", back_left_traction_joint_, "summit_xl_back_left_wheel_joint");
        controller_nh.param<std::string>("joint/back_right_wheel_joint/name", back_right_traction_joint_, "summit_xl_back_right_wheel_joint");
        
        controller_nh.param("joint/front_right_wheel_joint/max_speed", front_right_traction_joint_max_speed_, DEFAULT_MAX_JOINT_SPEED);
        controller_nh.param("joint/front_left_wheel_joint/max_speed", front_left_traction_joint_max_speed_, DEFAULT_MAX_JOINT_SPEED);
        controller_nh.param("joint/back_left_wheel_joint/max_speed", back_left_traction_joint_max_speed_, DEFAULT_MAX_JOINT_SPEED);
        controller_nh.param("joint/back_right_wheel_joint/max_speed", back_right_traction_joint_max_speed_, DEFAULT_MAX_JOINT_SPEED);
        
		ROS_INFO_STREAM_NAMED(controller_name_, controller_name_ << "::init: joints ["<< front_right_traction_joint_ << "," << front_left_traction_joint_ << "," << back_left_traction_joint_<< "," << back_right_traction_joint_ );
		           
        if (controller_nh.hasParam("linear_speed_limit") == false) { // limit does not exist!
            ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::init: Cannot find parameter linear_speed_limit parameter. I will gently set it to " << linear_speed_limit_);
        }
        else {
            controller_nh.param("linear_speed_limit", linear_speed_limit_, linear_speed_limit_);
            if (linear_speed_limit_ < 0) {
                ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::init: Watch out! You set linear_speed_limit, which is the limit of the modulo of the linear speed, to a negative value. I will gently set it as positive.");
                linear_speed_limit_ = -linear_speed_limit_;
            }
        }
        
        if (controller_nh.hasParam("linear_acceleration_limit") == false) { // limit does not exist!
            ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::init: Cannot find parameter linear_acceleration_limit parameter. I will gently set it to " << linear_acceleration_limit_);
        }
        else {
            ...
            if (linear_acceleration_limit_ < 0) {
                ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::init: Watch out! You set linear_acceleration_limit, which is the limit of the modulo of the linear acceleration, to a negative value. I will gently set it as positive.");
        ...
        if (controller_nh.hasParam("angular_speed_limit") == false) { // limit does not exist!
            ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::init: Cannot find parameter angular_speed_limit parameter. I will gently set it to " << angular_speed_limit_);
        }
        else {
            controller_nh.param("angular_speed_limit", angular_speed_limit_, angular_speed_limit_);
            if (angular_speed_limit_ < 0) {
                ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::init: Watch out! You set angular_speed_limit, which is the limit of the modulo of the angular speed, to a negative value. I will gently set it as positive.");
                angular_speed_limit_ = -angular_speed_limit_;
            }
        }
        
        if (controller_nh.hasParam("angular_acceleration_limit") == false) { // limit does not exist!
            ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::init: Cannot find parameter angular_acceleration_limit parameter. I will gently set it to " << angular_acceleration_limit_);
        }
        else {
            ...
            if (angular_acceleration_limit_ < 0) {
                ROS_WARN_STREAM_NAMED(controller_name_, controller_name_ << "::init: Watch out! You set angular_acceleration_limit, which is the limit of the modulo of the angular acceleration, to a negative value. I will gently set it as positive.");
		...
		// Advertise services
		srv_SetMode_ = root_nh.advertiseService("set_mode", &SummitXLController::srvCallback_SetMode, this);
        ...
        joint_limits_[FRONT_RIGHT_TRACTION_JOINT] = std::make_pair(-front_right_traction_joint_max_speed_, front_right_traction_joint_max_speed_);
        joint_limits_[FRONT_LEFT_TRACTION_JOINT] = std::make_pair(-front_left_traction_joint_max_speed_, front_left_traction_joint_max_speed_);
        joint_limits_[BACK_RIGHT_TRACTION_JOINT] = std::make_pair(-back_right_traction_joint_max_speed_, back_right_traction_joint_max_speed_);
        joint_limits_[BACK_LEFT_TRACTION_JOINT] = std::make_pair(-back_left_traction_joint_max_speed_, back_left_traction_joint_max_speed_);
       ...
       /* for (size_t i = BEGIN_TRACTION_JOINT; i < END_TRACTION_JOINT; i++) {
            ...
            if (controller_nh.hasParam(param_limit_name) == false) { // limit does not exist!
                ROS_ERROR_STREAM_NAMED(controller_name_, controller_name_ << "::init: cannot find parameter " << param_limit_name << ". It is required!");
                everything_ok = false;
            }
            else {
    ...
    void SummitXLController::updateJointReferences()
    {
        ...
        q[0] = (vx + vy + lab * w) / (wheel_diameter_/2.0);	 // divide by (wheel_diameter_/2.0) to convert from m/s to rad/s
	...
	if(fabs(brw4) < 0.001) brw4 = 0.0;
        ...
        //robot_pose_.x += cos(robot_pose_.theta) * vx * fSamplePeriod + cos(M_PI_2 + robot_pose_.theta) * vy * fSamplePeriod;
        //robot_pose_.y += sin(robot_pose_.theta) * vx * fSamplePeriod + sin(M_PI_2 + robot_pose_.theta) * vy * fSamplePeriod;
    ...
    {    
        ...
        bool zero_speed_ref = (received_cmd_.linear.x == 0) && (received_cmd_.linear.y == 0) && (received_cmd_.angular.z==0);    
    ...
    bool SummitXLController::srvCallback_SetMode(robotnik_msgs::set_mode::Request& request, robotnik_msgs::set_mode::Response& response)
	{
	...
	// Service GetMode
	bool SummitXLController::srvCallback_GetMode(robotnik_msgs::get_mode::Request& request, robotnik_msgs::get_mode::Response& response)
	{
	...
	// Service SetOdometry 
	bool SummitXLController::srvCallback_SetOdometry(robotnik_msgs::set_odometry::Request &request, robotnik_msgs::set_odometry::Response &response )
	{
		//ROS_INFO("SummitXLController::srvCallback_SetOdometry: request -> x = %f, y = %f, a = %f", request.x, request.y, request.orientation);
		...
		robot_pose_.theta = request.orientation;
                ...
                ROS_INFO("q.x=%5.2d q.y=%5.2f  q.z=%5.2f  q.w=%5.2f", q.x(), q.y(), q.z(), q.w() ); 
